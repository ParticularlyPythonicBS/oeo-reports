...
investment_rows = Array.from({length: investment_data.regions.length}, (_, i) =>
  Object.fromEntries(Object.entries(investment_data).map(([k, v]) => [k, v[i]]))
)

investment_region_set = Array.from(new Set(investment_rows.map(d => d.regions)))
investment_tech_set = Array.from(new Set(investment_rows.map(d => d.tech)))

viewof investment_region_view = Inputs.select(investment_region_set, {label: "region", multiple: true})
viewof investment_tech_view = Inputs.select(investment_tech_set, {label: "technology", multiple: true})


function makeInvestmentPlot(region_view, tech_view) {
  const filtered_data = investment_rows.filter(d =>
    (!investment_region_view || investment_region_view.length === 0 || investment_region_view.includes(d.regions)) &&
    (!investment_tech_view || investment_tech_view.length === 0 || investment_tech_view.includes(d.tech))
  )
...
makeInvestmentPlot(investment_region_view, investment_tech_view)
...
fixed_rows = Array.from({length: fixed_data.regions.length}, (_, i) =>
  Object.fromEntries(Object.entries(fixed_data).map(([k, v]) => [k, v[i]]))
)

fixed_region_set = Array.from(new Set(fixed_rows.map(d => d.regions)))
fixed_tech_set = Array.from(new Set(fixed_rows.map(d => d.tech)))

viewof fixed_region_view = Inputs.select(fixed_region_set, {label: "region", multiple: true})
viewof fixed_tech_view = Inputs.select(fixed_tech_set, {label: "technology", multiple: true})


function makeFixedCostPlot(region_view, tech_view) {
  const filtered_data = fixed_rows.filter(d =>
    (!fixed_region_view || fixed_region_view.length === 0 || fixed_region_view.includes(d.regions)) &&
    (!fixed_tech_view || fixed_tech_view.length === 0 || fixed_tech_view.includes(d.tech))
  )
  ...
  return Plot.plot({
      x: {tickFormat: "", label: "Vintage"},
      y: {label: "Fixed costs ($M/GW)"},
      color: {legend: true},
      marks: [
          Plot.barY(filtered_data, {x: "vintage", y: "cost_fixed", fill: "tech", strokeWidth: 2, tip: "x"})
      ],
  })
}

makeFixedCostPlot(fixed_region_view, fixed_tech_view)
...
variable_rows = Array.from({length: variable_data.regions.length}, (_, i) =>
  Object.fromEntries(Object.entries(variable_data).map(([k, v]) => [k, v[i]]))
)

variable_region_set = Array.from(new Set(variable_rows.map(d => d.regions)))
variable_tech_set = Array.from(new Set(variable_rows.map(d => d.tech)))

viewof variable_region_view = Inputs.select(variable_region_set, {label: "region", multiple: true})
viewof variable_tech_view = Inputs.select(variable_tech_set, {label: "technology", multiple: true})


function makeVariableCostPlot(region_view, tech_view) {
  const filtered_data = variable_rows.filter(d =>
    (!variable_region_view || variable_region_view.length === 0 || variable_region_view.includes(d.regions)) &&
    (!variable_tech_view || variable_tech_view.length === 0 || variable_tech_view.includes(d.tech))
  )
  ...
  return Plot.plot({
      x: {tickFormat: "", label: "Vintage"},
      y: {label: "Variable costs ($M/PJ)"},
      color: {legend: true},
      marks: [
          Plot.barY(filtered_data, {x: "vintage", y: "cost_variable", fill: "tech", strokeWidth: 2, tip: "x"})
      ],
  })
}

makeVariableCostPlot(variable_region_view, variable_tech_view)
...
capacity_rows = Array.from({length: capacity_data.regions.length}, (_, i) =>
  Object.fromEntries(Object.entries(capacity_data).map(([k, v]) => [k, v[i]]))
)

capacity_region_set = Array.from(new Set(capacity_rows.map(d => d.regions)))
capacity_tech_set = Array.from(new Set(capacity_rows.map(d => d.tech)))

viewof capacity_region_view = Inputs.select(capacity_region_set, {label: "region", multiple: false})
viewof capacity_tech_view = Inputs.select(capacity_tech_set, {label: "technology", multiple: false})


function makeCapacityPlot(region_view, tech_view) {
  const filtered_data = capacity_rows.filter(d =>
    (!capacity_region_view || capacity_region_view.length === 0 || capacity_region_view.includes(d.regions)) &&
    (!capacity_tech_view || capacity_tech_view.length === 0 || capacity_tech_view.includes(d.tech))
  )
  ...
  return Plot.plot({
      ...
      y: {label: "Capacity factor"},
      color: {legend: true},
      marks: [
          Plot.lineY(filtered_data,
                    {y: "cf_tech", stroke: "tech", strokeWidth: 2, tip: "x"})
      ],
  })
}

makeCapacityPlot(capacity_region_view, capacity_tech_view)
```
