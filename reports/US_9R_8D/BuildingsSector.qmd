...
rows = Array.from({length: data.demand_comm.length}, (_, i) =>
  Object.fromEntries(Object.entries(data).map(([k, v]) => [k, v[i]]))
)

demand_set = Array.from(new Set(rows.map(d => d.demand_comm)))

// viewof demand_view = Inputs.select([null].concat(demand_set), {label: "demand", multiple: true})
viewof demand_view = Inputs.select(demand_set, {label: "demand", multiple: true})

filtered_data = (demand_view && demand_view.length > 0)
  ? rows.filter(d => demand_view.includes(d.demand_comm))
  : rows
